...
# set the all given variables for RSA-OAEP
...
# try all possible 4 digit pins in the loop
for i in range(1000, 9999):
    # now try all possible 8 bit unsigned integers used as random number in OAEP
    for j in range(256):
        # if the encryption matches the ciphertext, we get the correct i value as the random pin
        if RSA_OAEP_Enc(i, e, N, j) == c:
            # The random value is 246
            print("The random value is:", j)
            m = i
            break
# The pin is: 5718
print("The pin is :", m)
